...
class PrefsManager(
    ...
    // ---- Reading goal ----
    var readingGoalMinutesPerDay: Int
        get() = prefs.getInt(KEY_GOAL_MINUTES, 30)
        set(value) {
            prefs.edit().putInt(KEY_GOAL_MINUTES, value).apply()
        }

    var showReadingGoal: Boolean
        get() = prefs.getBoolean(KEY_SHOW_GOAL, true)
        set(value) {
            prefs.edit().putBoolean(KEY_SHOW_GOAL, value).apply()
        }
    ...
    companion object {
        ...
        const val KEY_GOAL_MINUTES = "reading_goal_minutes"
        const val KEY_SHOW_GOAL = "show_reading_goal"
