...
import com.bookwormbliss.app.databinding.ActivityMainBinding
import com.bookwormbliss.app.epub.BookFileTypes
import com.bookwormbliss.app.epub.EpubImporter
import com.bookwormbliss.app.epub.MetadataRefreshReportStore
import com.bookwormbliss.app.epub.RescanDecision
import com.bookwormbliss.app.ui.BookAdapter
import com.bookwormbliss.app.ui.BookshelfViewModel
import com.bookwormbliss.app.ui.DisplayItem
import com.bookwormbliss.app.ui.DrawerAdapter
import com.bookwormbliss.app.ui.DrawerItem
import com.bookwormbliss.app.ui.HomeBookAdapter
import com.bookwormbliss.app.ui.HomeContent
import com.bookwormbliss.app.ui.RowAdapter
import com.bookwormbliss.app.util.CurrentlyReadingUndoSnackbar
import com.bookwormbliss.app.ui.ShelfView
...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var keepScreenOnController: com.bookwormbliss.app.util.KeepScreenOnController
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        keepScreenOnController = com.bookwormbliss.app.util.KeepScreenOnController(this, prefs)
    ...
    private fun drawerItems(): List<DrawerItem> =
        listOf(
            ...
            DrawerItem(getString(R.string.nav_vocabulary), R.drawable.ic_vocabulary, launchActivity = com.bookwormbliss.app.ui.VocabularyActivity::class.java),
    ...
    private fun bindHomeGroups(
        ...
        groups: List<com.bookwormbliss.app.ui.HomeGroup>,
    ) {
    ...
    private fun bindRowAdapter(rows: List<DisplayItem.GroupRow>) {
        ...
        if (scrollToTopOnNextContent) {
            ...
            newRowAdapter.submitList(rows.map { com.bookwormbliss.app.data.GroupedRow(it.name, it.count) }) {
                binding.recycler.post {
                    (binding.recycler.layoutManager as? LinearLayoutManager)
                        ?.scrollToPositionWithOffset(0, 0)
                    binding.recycler.itemAnimator = previousAnimator
    ...
    private fun showSettingsView() {
        ...
        // 1. App Theme selector (Original / Pastel)
        addSettingsSectionHeader(getString(R.string.settings_app_theme))
        addSettingsSubtext(getString(R.string.settings_app_theme_summary))
        addAppThemeSelector()

        // 2. Daily Reading Goal
        addSettingsSectionHeader(getString(R.string.settings_reading_goal))
        addSettingsSubtext(getString(R.string.settings_reading_goal_summary))
        addReadingGoalSection()

        // 3. Reader Color Theme (opens reader settings)
        addSettingsSectionHeader(getString(R.string.settings_reader_color_theme))
        addSettingsRow(getString(R.string.settings_reader_color_theme), readerThemeLabel()) {
            startActivity(Intent(this, com.bookwormbliss.app.ui.ReaderSettingsActivity::class.java))
        }

        // 4. Typography & Sizing (opens reader settings)
        addSettingsSectionHeader(getString(R.string.settings_typography))
        addSettingsRow(getString(R.string.settings_font), prefs.font) {
            startActivity(Intent(this, com.bookwormbliss.app.ui.ReaderSettingsActivity::class.java))
        }

        // 5. Full Backup & Restore
        addSettingsSectionHeader(getString(R.string.settings_backup_restore))
        addSettingsSubtext(getString(R.string.settings_backup_summary))
        addBackupRestoreSection()

        // 6. Maintenance & Search History
        addSettingsSectionHeader(getString(R.string.settings_maintenance))
        addMaintenanceSection()

        // 7. Keep Screen On toggle
        addSettingsSectionHeader(getString(R.string.settings_keep_screen_on))
        addScreenOnToggle()

        // 8. Privacy Guarantee & Architecture
        addPrivacySection()
    }

    private fun addSettingsSectionHeader(title: String) {
        ...
        binding.emptyState.addView(tv)
        dynamicEmptyChildren.add(tv)
    }

    private fun addSettingsSubtext(text: String) {
        val tv = android.widget.TextView(this).apply {
            this.text = text
    ...
    private fun addAppThemeSelector() {
        val row = android.widget.LinearLayout(this).apply {
            orientation = android.widget.LinearLayout.HORIZONTAL
            gravity = android.view.Gravity.CENTER_VERTICAL
            setPadding(0, 8.dp(), 0, 8.dp())
        }
        val btnOriginal = com.google.android.material.button.MaterialButton(this).apply {
            text = getString(R.string.settings_theme_original)
            val isOriginal = prefs.appTheme == PrefsManager.AppTheme.ORIGINAL
            if (isOriginal) {
                ...
                setTextColor(themeColor(android.R.attr.textColorPrimary))
                strokeWidth = 1
                strokeColor = themeColor(android.R.attr.textColorSecondary)
            }
            setOnClickListener {
                prefs.appTheme = PrefsManager.AppTheme.ORIGINAL
                recreate()
            }
        }
        val btnPastel = com.google.android.material.button.MaterialButton(this).apply {
            text = getString(R.string.settings_theme_pastel)
            val isPastel = prefs.appTheme == PrefsManager.AppTheme.PASTEL
            if (isPastel) {
                setBackgroundColor(themeColor(com.google.android.material.R.attr.colorPrimary))
                setTextColor(android.graphics.Color.WHITE)
            } else {
            ...
            setOnClickListener {
                prefs.appTheme = PrefsManager.AppTheme.PASTEL
                recreate()
            }
        }
        row.addView(btnOriginal)
        row.addView(btnPastel)
        binding.emptyState.addView(row)
        dynamicEmptyChildren.add(row)
    }

    private fun addReadingGoalSection() {
        val container = android.widget.LinearLayout(this).apply {
            orientation = android.widget.LinearLayout.VERTICAL
            setPadding(0, 8.dp(), 0, 8.dp())
        }
        // Minutes label + value
        val minsRow = android.widget.LinearLayout(this).apply {
            orientation = android.widget.LinearLayout.HORIZONTAL
            gravity = android.view.Gravity.CENTER_VERTICAL
        }
        android.widget.TextView(this).apply {
            text = getString(R.string.settings_goal_minutes)
            setTextColor(themeColor(android.R.attr.textColorPrimary))
            textSize = 14f
            minsRow.addView(this)
        }
        val minsValue = android.widget.TextView(this).apply {
            text = "${prefs.readingGoalMinutesPerDay} mins"
            setTextColor(themeColor(com.google.android.material.R.attr.colorPrimary))
            textSize = 14f
            setTypeface(typeface, android.graphics.Typeface.BOLD)
            minsRow.addView(this)
        }
        container.addView(minsRow)
        // Minute selector buttons
        val btnRow = android.widget.LinearLayout(this).apply {
            orientation = android.widget.LinearLayout.HORIZONTAL
            setPadding(0, 8.dp(), 0, 8.dp())
        }
        for (mins in intArrayOf(15, 20, 30, 45, 60)) {
            val btn = com.google.android.material.button.MaterialButton(this).apply {
                text = "${mins}m"
                val isSelected = prefs.readingGoalMinutesPerDay == mins
                if (isSelected) {
                    setBackgroundColor(themeColor(com.google.android.material.R.attr.colorPrimary))
                    setTextColor(android.graphics.Color.WHITE)
                } else {
                ...
                setOnClickListener {
                    prefs.readingGoalMinutesPerDay = mins
                    minsValue.text = "$mins mins"
                    showSettingsView()
                }
            }
            btnRow.addView(btn)
        ...
        // Show on stats toggle
        val toggleRow = android.widget.LinearLayout(this).apply {
            orientation = android.widget.LinearLayout.HORIZONTAL
            gravity = android.view.Gravity.CENTER_VERTICAL
            setPadding(0, 8.dp(), 0, 0)
        }
        val labelCol = android.widget.LinearLayout(this).apply {
            orientation = android.widget.LinearLayout.VERTICAL
            layoutParams = android.widget.LinearLayout.LayoutParams(0, android.widget.LinearLayout.LayoutParams.WRAP_CONTENT).apply { weight = 1f }
        }
        android.widget.TextView(this).apply {
            text = getString(R.string.settings_goal_show_stats)
            setTextColor(themeColor(android.R.attr.textColorPrimary))
            textSize = 14f
            labelCol.addView(this)
        }
        android.widget.TextView(this).apply {
            text = getString(R.string.settings_goal_show_stats_summary)
            setTextColor(themeColor(android.R.attr.textColorSecondary))
            textSize = 12f
            labelCol.addView(this)
        }
        val switch = androidx.appcompat.widget.SwitchCompat(this).apply {
            isChecked = prefs.showReadingGoal
        }
        switch.setOnCheckedChangeListener { _, isChecked ->
            prefs.showReadingGoal = isChecked
        }
        toggleRow.addView(labelCol)
        toggleRow.addView(switch)
        container.addView(toggleRow)
        binding.emptyState.addView(container)
        dynamicEmptyChildren.add(container)
    }

    private fun addBackupRestoreSection() {
        val container = android.widget.LinearLayout(this).apply {
            orientation = android.widget.LinearLayout.VERTICAL
            setPadding(0, 8.dp(), 0, 8.dp())
        }
        ...
        val btnExport = com.google.android.material.button.MaterialButton(this).apply {
            text = getString(R.string.settings_export_backup)
            setOnClickListener {
                lifecycleScope.launch(Dispatchers.IO) {
                    try {
                        val db = com.bookwormbliss.app.data.AppDatabase.get(applicationContext)
                        val backupManager = com.bookwormbliss.app.data.BackupRestoreManager(
                        ...
                        withContext(Dispatchers.Main) {
                            Snackbar.make(binding.root, e.message ?: "Export failed", Snackbar.LENGTH_LONG).show()
                        }
                    }
                }
            }
        ...
        val btnImport = com.google.android.material.button.MaterialButton(this).apply {
            text = getString(R.string.settings_import_backup)
            setOnClickListener {
    ...
    private fun shareBackupFile(file: java.io.File) {
        ...
        startActivity(Intent.createChooser(shareIntent, getString(R.string.settings_export_backup)))
    }
    ...
    private val backupRestoreLauncher = registerForActivityResult(
        ...
        if (uri != null) {
            lifecycleScope.launch(Dispatchers.IO) {
                try {
                    val tempFile = java.io.File.createTempFile("restore_backup", ".json", cacheDir)
                    contentResolver.openInputStream(uri)?.use { input ->
                        tempFile.outputStream().use { input.copyTo(it) }
                    } ?: throw java.io.IOException("Cannot read backup file")
                    val db = com.bookwormbliss.app.data.AppDatabase.get(applicationContext)
                    val backupManager = com.bookwormbliss.app.data.BackupRestoreManager(
                        applicationContext, db, prefs
                    )
                    backupManager.importFromFile(tempFile, com.bookwormbliss.app.data.BackupRestoreManager.ImportMode.MERGE)
                    tempFile.delete()
                    withContext(Dispatchers.Main) {
                        Snackbar.make(binding.root, getString(R.string.settings_restore_success), Snackbar.LENGTH_LONG).show()
                        recreate()
                    }
                } catch (e: Exception) {
                    withContext(Dispatchers.Main) {
                        Snackbar.make(binding.root, getString(R.string.settings_restore_failed, e.message ?: ""), Snackbar.LENGTH_LONG).show()
                    }
                }
            }
        }
    ...
    private fun addMaintenanceSection() {
        val container = android.widget.LinearLayout(this).apply {
            orientation = android.widget.LinearLayout.VERTICAL
            setPadding(0, 8.dp(), 0, 8.dp())
        }
        val btnClearHistory = com.google.android.material.button.MaterialButton(this).apply {
            text = getString(R.string.settings_clear_search_history)
            setOnClickListener {
                lifecycleScope.launch(Dispatchers.IO) {
                    val searchHistoryManager = com.bookwormbliss.app.data.SearchHistoryManager(applicationContext)
                    searchHistoryManager.clearAll()
                    withContext(Dispatchers.Main) {
                        Snackbar.make(binding.root, getString(R.string.settings_search_history_cleared), Snackbar.LENGTH_SHORT).show()
                    }
                }
            }
        }
    ...
    private fun addPrivacySection() {
        ...
        android.widget.TextView(this).apply {
            text = getString(R.string.settings_privacy_title)
            setTextColor(themeColor(com.google.android.material.R.attr.colorPrimary))
            textSize = 14f
            setTypeface(typeface, android.graphics.Typeface.BOLD)
        ...
        android.widget.TextView(this).apply {
            text = getString(R.string.settings_privacy_body)
            setTextColor(themeColor(android.R.attr.textColorPrimary))
            textSize = 12f
            setPadding(0, 8.dp(), 0, 4.dp())
            container.addView(this)
        }
        for (noteRes in intArrayOf(R.string.settings_privacy_note1, R.string.settings_privacy_note2, R.string.settings_privacy_note3)) {
            android.widget.TextView(this).apply {
    ...
    private fun readerThemeLabel(): String =
        getString(com.bookwormbliss.app.ui.ReaderTheme.byId(prefs.theme).displayNameRes)
