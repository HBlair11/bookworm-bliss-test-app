...
/**
 * ReaderPaginationScriptBuilder — Extracted from ReaderActivity (Phase 6).
 *
 * Builds the pagination JavaScript injected into the EPUB WebView. This is a
 * pure function with no Android lifecycle or View dependencies: it takes the
 * guard sizes and produces a JavaScript string.
 *
 * Extraction rationale: ReaderActivity had become a 5,118-line god activity.
 * The pagination script is a self-contained string builder with no interaction
 * state, making it the safest first extraction alongside [ReaderCssBuilder]
 * (minimal lifecycle risk).
 *
 * Key design: columns are `column-gap = 2*margin` wide and each page advances
 * by the full viewport width (innerWidth), so column N+1 begins exactly at
 * the right edge of the viewport and never peeks into page N. Bottom guard
 * shrinks body height to reserve space for the page indicator. Top guard
 * reserves symmetric breathing room below the status bar.
 *
 * The script exposes a `window.Caesura` object with:
 * - `apply()` — recalculate column layout for current viewport
 * - `pageCount()` / `currentPage()` — pagination queries
 * - `gotoPage(page, animate)` / `nextPage(animate)` / `prevPage(animate)` — navigation
 * - `ratio()` — reading progress fraction (0..1)
 * - `gotoElementById(id)` / `pageForElementById(id)` — anchor navigation
 * - `pageForTextAnchor(anchor, fallbackPage)` — text-based position restore
 * - `pageForWholePageAnchor(anchor, fallbackPage)` — snippet-based position restore
 * - `pageSnippet(page)` — extract a text snippet for a given page (for bookmarks)
 * - `setSelectionPageLock(active, page)` — prevent column auto-scroll during selection
 * - `highlightPageById(id)` / `gotoHighlightById(id)` — highlight navigation
 */
object ReaderPaginationScriptBuilder {

    /**
     * Build the pagination JavaScript for injection into the EPUB content WebView.
     *
     * @param guardPx    Bottom guard in pixels (space reserved for page indicator).
     * @param topGuardPx Top guard in pixels (breathing room below status bar).
     * @return A `<script>...</script>` block.
     */
