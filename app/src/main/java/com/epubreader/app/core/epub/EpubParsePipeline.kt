...
import com.epubreader.app.epub.EpubPageMap
import java.io.File
 ...
 *
 * This replaces the monolithic [com.epubreader.app.epub.EpubParser] with
...
object EpubParsePipeline {
    ...
    private fun estimatePageCounts(file: File, spine: EpubSpine): IntArray {
        ...
        return try {
            val tempBook = com.epubreader.app.epub.EpubBook(
                file = file,
                metadata = com.epubreader.app.epub.EpubMetadata(),
                manifest = emptyMap(),
                spine = spine.items.map {
                    com.epubreader.app.epub.SpineItem(it.idref, it.href, it.mediaType, it.linear)
                },
                toc = emptyList(),
                coverHref = null,
                opfDir = "",
